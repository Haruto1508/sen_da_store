...
public class OrderController {
    ...
    public ResponseEntity<Map<String, Object>> createOrder(@RequestBody CreateOrderRequest request) {
        ...
        if (result.containsKey("vietQr")) {
            response.put("vietQr", result.get("vietQr"));
        }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getOrderByCode(@PathVariable String orderCode) {
        Map<String, Object> orderData = orderService.getOrderByCode(orderCode);
        return ResponseEntity.ok(ApiResponse.ok("Lấy thông tin đơn hàng thành công", orderData));
    }

    @GetMapping("/customer")
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getOrdersByCustomer(@RequestParam String phone) {
        List<Map<String, Object>> list = orderService.getOrdersByCustomer(phone);
        return ResponseEntity.ok(ApiResponse.ok("Tải danh sách đơn hàng thành công", list));
    }
}
